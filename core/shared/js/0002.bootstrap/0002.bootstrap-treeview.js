...
;(function($, window, document, undefined) {
	...
	Tree.prototype = {
		...
		_triggerNodeSelectedEvent: function(node,state) { // <- Added 'state' by CP
			this.$element.trigger('nodeSelected', [$.extend(true, {}, node),state]);  // <- Added 'state' by CP
		},
		...
		_setSelectedNode: function(node) {
			...
			if (node === this.selectedNode) {
                            this._triggerNodeSelectedEvent(this.selectedNode = node,'inactive'); // <- added state 'active' by CP
                            this.selectedNode = {};
			}
			else {
				this._triggerNodeSelectedEvent(this.selectedNode = node,'active'); // <- added state 'inactive' by CP
			}
		...
		_buildTree: function(nodes, level) {
			...
			$.each(nodes, function addNodes(id, node) {
				...
				var treeItem = $(self._template.item)
					...
					.attr('data-related-to', node.relatedTo) // Added by CP
