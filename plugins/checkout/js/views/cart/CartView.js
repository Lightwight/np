...
np.view.extend ('CartView', (function () {
    ...
    return {
        ...
        amountChanged: function (model) {
            var cartArticles, cachedArticle, cArticle, cAmount, artID, artName, currentArticles, cID, cName,
                cRows, fRow, id,
            ...
            for (i=0; i<l; i++) {
                ...
                if (typeof currentArticles[cName] !== 'undefined') {
                    ...
                    if (fRow) {
                        ...
                        cachedArticle   = np.Cart.getCachedProduct (cArticle.article_id);
                        
                        if (typeof cachedArticle['changing'] === 'undefined' || !cachedArticle['changing']) {
                            np.observable.update ('CartProduct', id, 'amount', cAmount);
                        }
                    } else {
                        // Article isn't in cart. We have to add it:
                        container   = $('#cart .cart-category-container[data-category-id="'+cID+'"]');
                    ...
                    $('#cart .cart-category-container').each (function (row) {
                        var _this, tName, tID, 
                    ...
                    if (!inserted) {
                        $('#cart .cart-outer-container').append (htmlCategory);
                    }
                }
            }
