...
np.controller.extend ('ProductSpreadVariationsController', {
   view:    'ProductSpreadVariationsView',
   model:   function () {
   ...
   events: {
        setVariation: function (view) {
            ...
            if (!np.client.isMobile ()) {
                view.rerender ('ProductListView', true, 'Product', _ctx.get ('id'), np.jsonClone (_ctx.getAll ()));
            }
        }
   }
});