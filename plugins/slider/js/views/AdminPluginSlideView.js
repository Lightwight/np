...
np.view.extend ('AdminPluginSlideView', {
    ...
    showError: function (model) {
        ...
        if (code !== false) {
            ...
            message     = 'Der Slide konnte nicht gespeichert werden.<br><br>';
    ...
    changedOrder: function (model) {
        ...
        if (position !== order) {
            np.observable.update ('AdminPluginSlide', model.get ('id'), 'order', position);
        }
    }.observes ('changed_order').on ('change'),

    enableSlideDown: function (model) {
        var len, order;

        len     = $('#admin-plugin-slider-sortarea .admin-plugin-slider-edit').length;
        order   = model.get ('order');
    ...
    orderChanged: function (model) {
        ...
        position    = this.index () + 1;
        order       = model.get ('order');
        len         = this.parents ('*:first').find ('.admin-plugin-slider-edit').length;
        _t          = this;
        ...
        if (position !== order) {
            ...
            if (position > order) {
                this.insertBefore (container.find ('.admin-plugin-slider-edit:eq('+(order-1)+')'));
            } else {
                this.insertAfter (container.find ('.admin-plugin-slider-edit:eq('+(order-1)+')'));
            }
        }
    }.observes ('order').on ('change'),
    ...
    deleted: function (model) {
        ...
        if (model.get ('successDeleted') === true) {
            ...
            message     = 'Der Slide konnte nicht gelöscht werden.<br><br>';
            message    += 'Bitte überprüfen Sie Ihre Internetverbindung und wiedeholen Sie den Vorgang.<br><br>';
            message    += 'Sollte der Fehler erneut auftauchen, dann setzen Sie sich bitte mit Ihrem Systemadministrator in Verbindung.';
