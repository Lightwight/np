...
class UpdateHandler extends HandlerHelper
{
    ...
    public function resolveUpdate ($params)
    {
        if ($this->method === 'get')           { return $this->fetchUpdate ($params);   }
        else if ($this->method === 'post')     { return $this->postUpdate ($params);    }
    }
    
    public function fetchUpdate ($params)
    {
        $isCheckForUpdate   = isset ($params['update']['checkForUpdate']);
        $isUpdate           = isset ($params['update']['update']);
        
        if ($isCheckForUpdate)
        {
            $controller     = new UpdateController ();
            $result         = $controller->getNewVersion ();
        }
        else if ($isUpdate)
        {
            $controller     = new UpdateController ();
