...
np.controller.extend ('AdminRoutesOverviewSearchController', {
    view:   'AdminRoutesOverviewSearchView',
    model:  function () {
        return {
            AdminRoutesSearch: {
                id:         -1,
    ...
    events: {
        search: function (view) {
            ...
            if (search.length > 0) {
                ...
                sortBy      = sort.sortBy ? '/'+sort.sortBy : '/title';
                sortOrder   = sort.sortOrder ? '/'+sort.sortOrder : '/asc';

                this.set ('sentSearch', search);

                np.model.Routes.flush ();
                np.routeTo ('#/admin/routesmanagement/routes/1'+sortBy+sortOrder+'/'+search);
            }
        },
        ...
        clearSearch: function () {
            ...
            np.model.Routes.flush ();
            np.routeTo ('#/admin/routesmanagement/routes/1'+sortBy+sortOrder);
        }
    }
});