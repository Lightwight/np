...
class RoutesController extends ControllerHelper implements ControllerInterface
{
    public function getModel (\Model $model, $params) 
    {
        ...
        if ($this->isGroup (1))
        {
            if (!$search)
            {
                $routes = $model->paginate (10, $page)->findBy ('customizable', 1)->orderBy ($sort, $sortOrder)->result ();
            }
            else
            {
                $routes = $model->paginate (10, $page)->findBy ('customizable', 1)->orderBy ($sort, $sortOrder)->findLike ('ID', $search)->orFindLike ('title', $search)->orFindLike ('route', $search)->orFindLike ('scope_id', $search)->result ();
            }        
            
            $mScopes    = new Model ('Route_scopes');
            ...
            if (is_array ($routes))
            {
                foreach ($routes as $route)
                {
                    ...
                    $scope  = $mScopes->findBy ('ID', $route->get ('scope_id'))->result ();

                    $route->set ('scope', $scope->get ('name'));
                }
            }            
        }
