...
class Article_unitsController extends ControllerHelper implements ControllerInterface 
{
    ...
    public function postModel (\Model $model)           
    {
        $row        = $model->result ();
        $name       = $row->get ('name');
        $unitID     = $model->getNext ('unit_id');
        if ($unitID < 1000) { $unitID = 1000;   }
        
        $postID     = $model->add (array ('unit_id' => $unitID, 'name'=> $name))->result ()->post ();
